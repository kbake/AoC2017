passphrase_list_str = """bdwdjjo avricm cjbmj ran lmfsom ivsof
...
# Part 1
num_valid = 0
for row in passphrase_list_str.split('\n'):
	row_list = []
	is_valid = True
	for chunk in row.split():
		if chunk in row_list:
			is_valid = False
			break	
	...
	for chunk in row.split():
		...
		for c in chunk:
			if c in broken_down:
				...
				broken_down[c] = 1
		if broken_down in row_list:
			is_valid = False
			break
		else:
			row_list.append(broken_down)
	if is_valid:
		num_valid += 1
print(num_valid)
