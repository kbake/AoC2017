...
# Part 1
def find_bottom(input_str):
  input_list = input_str.split('\n')
  relevant_items = [x for x in input_list if "->" in x]
  names_list = []
  holding_list = []
  for item in relevant_items:
    split_item = item.split("->")
    names_list.append(split_item[0].split()[0].strip())
    [holding_list.append(x.strip()) for x in split_item[1].split(", ")]
