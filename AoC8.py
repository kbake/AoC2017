...
def find_largest_register(input_text):
  registers = {}
  overall_max = 0
  for command in input_text.split('\n'):
    split_command = command.split("if")
    beginning = split_command[0].strip()
    condition = split_command[1].strip()
    register_to_edit = beginning.split()[0]
    should_increase = beginning.split()[1] == "inc"
    amount_modify = int(beginning.split()[2])
    if register_to_edit not in registers:
      registers[register_to_edit] = 0
    split_condition = condition.split()
    condition_register = split_condition[0]
    condition_compare = split_condition[1]
    condition_value = int(split_condition[2])
    if condition_register not in registers:
      registers[condition_register] = 0
    valid_condition = is_condition_valid(registers, condition_register, condition_compare, condition_value)
    if valid_condition:
      if should_increase:
        registers[register_to_edit] += amount_modify
      else:
        registers[register_to_edit] -= amount_modify
      if registers[register_to_edit] > overall_max:
        overall_max = registers[register_to_edit]
  max = 0
  max_register = ""
  for reg in registers:
    if registers[reg] >= max:
      max = registers[reg]
      max_register = reg
  print(max_register + ": " + str(max))
  print(overall_max)

def is_condition_valid(register_dict, focus_register, conditional, value):
  if conditional == ">":
    if register_dict[focus_register] > value:
      return True
  elif conditional == "==":
    if register_dict[focus_register] == value:
      return True
    ...
    return False
find_largest_register(sample)

main_input = """j inc -19 if jhb >= 10
es inc -432 if gbu <= -5
es inc 278 if ib > -9
es inc -835 if ib >= -6
q dec 420 if buw == -2
...
find_largest_register(main_input)